...
type Channel struct {
	...
	messages []*message.Message      // Slice with messages sent to chat
...
func NewChannel(name string) *Channel {
	...
	messages := make([]*message.Message, 5, 5)
	return &Channel{
		name:     name,
		users:    users,
		messages: messages,
...
func (c *Channel) AddMessage(sender *user.User, content string) *message.Message {
...
func (c *Channel) SendMessage(sender *user.User, msg *message.Message) {
...
func (c *Channel) Notify(sender *user.User, msg *message.Message) {
	c.Call(func(n net.Conn, u *user.User) {
		if u != sender {
			w := n.(io.Writer)
			io.WriteString(w, msg.Notify())
		}
	})
}
