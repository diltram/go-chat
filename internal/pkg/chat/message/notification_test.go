...
func TestStringNotification(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			"10/18/2018 10:00:00 | Notification!\r\n",
		},
		{
			"john",
			"How are you brother?",
			time.Date(2018, time.January, 1, 1, 0, 0, 0, time.UTC),
			"01/1/2018 01:00:00 | How are you brother?\r\n",
		},
	}
	...
	for i, tt := range tests {
		...
		msg := NewNotification(sender, tt.content, tt.date)

		if msg.String() != tt.expString {
			t.Errorf("Notification(%d): expected %s, actual %s", i, tt.expString, msg.String())
		}
	}
}
