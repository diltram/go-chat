...
// Notify returns textual representation of a message in notify style.
// It includes information like date and notifiction text.
func (m *Message) Notify() string {
	return fmt.Sprintf("%s | %s", m.date.Format("03/2/2006 15:04:05"), m.text)
}

// NewMessage generates a message with information about the user, content and
// time.
func NewMessage(sender *user.User, content string, date time.Time) *Message {
	return &Message{
		date:   date,
		text:   content,
		sender: sender,
